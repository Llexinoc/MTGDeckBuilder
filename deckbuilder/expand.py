...
def _datamuse_expand(query: str, no_network: bool = False) -> list[str]:
    if not query or requests is None or no_network:
        if no_network:
            logger.debug(f"Skipped Datamuse expansion for '{query}' (no_network=True)")
        return []
    ...
    try:
        ...
        logger.debug(f"Datamuse expansion failed for '{query}': {e}")
        return []


def _llm_expand(query: str, no_network: bool = False) -> list[str]:
    api_key = os.environ.get("ANTHROPIC_API_KEY")
    if not api_key or requests is None or no_network:
        if no_network:
            logger.debug(f"Skipped LLM expansion for '{query}' (no_network=True)")
        return []
    try:
        prompt = (
...
def expand_query(query: str, use_llm: bool = True, no_network: bool = False) -> dict:
    """Expand a short query into related concepts and MTG themes.
    
    Args:
        query: The user's theme description
        use_llm: Whether to use LLM enrichment
        no_network: If True, skip all network requests (Datamuse, LLM)
    """
    ...
    datamuse_words = _datamuse_expand(cleaned, no_network=no_network)
    llm_words = _llm_expand(cleaned, no_network=no_network) if use_llm else []
