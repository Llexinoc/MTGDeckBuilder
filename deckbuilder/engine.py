...
def build_deck(description: str, fmt: str = "commander", offline: bool = False,
    ...
    # Try to use card index
    db_conn = None
    try:
        db_conn = carddata.connect()
    except FileNotFoundError:
        # Index not available; use live API or sample pool
        pass
    
    try:
        deck = DeckBuilder(client, fmt=resolved_fmt, db_conn=db_conn).build(params)
        # Add sources metadata
        deck["sources"] = {
            "cards": client.card_source,
            "network": not no_network,
            "llm": use_llm and not no_network,
        }
        return deck
    finally:
        if db_conn:
