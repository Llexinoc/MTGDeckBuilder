...
document.addEventListener("DOMContentLoaded", () => {
  fetch("/api/health").then(r => r.json()).then(h => {
    $("llm-badge").textContent = h.llm?.available ? "LLM enrichment: on" : "Logic engine: heuristic";
  }).catch(() => {});
...
function cardTile(c) {
  // Build image element with lazy loading and error fallback
  let imgHtml;
  if (c.image) {
    // Create a unique fallback ID for this card
        ...
        if (fb) {
          fb.replaceWith(
            Object.assign(document.createElement('div'), {
              className: 'noimg',
              innerHTML: '<strong>${escapeAttr(c.name)}</strong><br><small>${escapeAttr(c.type_line || '')}</small>'
            })
          );
        }
      "
    ...
    // No image data available - render nameframe
    imgHtml = `<div class="noimg"><strong>${escapeHtml(c.name)}</strong><br><small>${escapeHtml(c.type_line || "")}</small></div>`;
  }

  // Card link with quantity
  const qty = (c.count && c.count > 1) ? `${c.count}× ` : "";
  const link = c.uri
    ? `<a href="${c.uri}" target="_blank" rel="noopener" title="${escapeHtml(c.name)}">${qty}${escapeHtml(c.name)}</a>`
    : `<span title="${escapeHtml(c.name)}">${qty}${escapeHtml(c.name)}</span>`;

  return `<div class="mcard" title="${escapeHtml(c.name)}">${imgHtml}<div class="cname">${link}<span class="cmc">${manaText(c.mana_cost)}</span></div></div>`;
}
