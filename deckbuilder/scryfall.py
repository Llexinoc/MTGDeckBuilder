...
def _cached_get(url: str, params: dict) -> dict | None:
    ...
    if resp.status_code == 429:
        raise RuntimeError("Scryfall rate limit hit (429). Back off 30s and retry.")
...
class ScryfallClient:
    ...
    def find_cards(
        ...
        try:
            collected: list[dict] = []
            data = _cached_get(f"{API_BASE}/cards/search", params)
            ...
            # Graceful degrade to the bundled pool so a demo always works.
            self.offline_fell_back = True
            self.last_source = "offline"
            return self._offline_find(color_identity, card_type, oracle_terms,
                                      name_terms, exclude_types, max_cmc, limit)
    ...
    def resolve_set_code(self, set_name: str | None) -> str | None:
        ...
        if self.offline or requests is None:
            return None
        try:
            data = _cached_get(f"{API_BASE}/sets", {})
            if not data:
                return None
    ...
    def resolve_named_commander(self, name, fmt="commander", set_codes=None):
        ...
        params = {"fuzzy": name}
        try:
            data = _cached_get(f"{API_BASE}/cards/named", params)
            if not data:
                return None
            card = normalize(data)
